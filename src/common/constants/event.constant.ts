// post
export const PostWillBeCreated = 'post.will.be.created';
export const PostWillBeUpdated = 'post.will.be.updated';
export const PostWillBeDeleted = 'post.will.be.deleted';

export const PostHasBeenCreated = 'post.has.been.created';
export const PostHasBeenUpdated = 'post.has.been.updated';
export const PostHasBeenDeleted = 'post.has.been.deleted';

// post
export const CommentWillBeCreated = 'comment.will.be.created';
export const CommentWillBeUpdated = 'comment.will.be.updated';
export const CommentWillBeDeleted = 'comment.will.be.deleted';

export const CommentHasBeenCreated = 'comment.has.been.created';
export const CommentHasBeenUpdated = 'comment.has.been.updated';
export const CommentHasBeenDeleted = 'comment.has.been.deleted';

// user
export const UserHasJoinedGroup = 'user.has.joined.group';
export const UserHasLeftGroup = 'User.Has.left.group';
export const UserHasJoinedCommunity = 'user.has.joined.community';
export const UserHasLeftCommunity = 'user.has.left.community';

// file
export const FileHasBeenUploaded = 'file.has.been.uploaded';

// reaction
export const ReactionHasBeenCreated = 'reaction.has.been.created';
export const ReactionHasBeenRemoved = 'reaction.has.been.removed';

// ws
export const OnWebSocketConnect = 'on.websocket.connect';
export const OnWebSocketDisconnect = 'on.websocket.disconnect';
