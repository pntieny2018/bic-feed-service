...
describe('DeleteReactionHandler', () => {
  ...
  describe('execute', () => {
    it('should delete reaction success', async () => {
      ...
      const command = new DeleteReactionCommand({ reactionName: '+1', target: target, targetId: targetId, userId: userMock.id });
      const reactionRecord = { id: v4(), createdBy: userMock.id, targetId, target: target, reactionName: '+1' };
