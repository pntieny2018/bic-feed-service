...
describe('FindTagsPaginationHandler', () => {
  ...
  describe('execute', () => {
    it('should find tags success', async () => {
      ...
      const groupChildrenData = { id: v4(), name: 'group2', slug: 'group2', child: { open: [], private: [], closed: [], secret: [] }};
      ...
      jest.spyOn(groupAppService, 'findAllByIds')
      ...
      expect(result).toEqual({ rows: tags.map(tag => ({
        id: tag.id,
        groupId: tag.groupId,
        name: tag.name,
        slug: tag.slug,
