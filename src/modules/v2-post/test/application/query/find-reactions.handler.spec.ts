...
describe('FindReactionsPaginationHandler', () => {
  ...
  describe('execute', () => {
    it('should find reactions success', async () => {
      ...
      jest.spyOn(reactionQuery, 'getPagination').mockResolvedValue({ rows: reactionEntities, total: 2 });
      jest.spyOn(userAppService, 'findAllByIds').mockResolvedValue([{ id: createdBy }, { id: createdBy2 }]);
