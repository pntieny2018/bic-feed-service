...
import {
  ...
  IPostReactionRepository, POST_REACTION_REPOSITORY_TOKEN,
} from '../../../domain/repositoty-interface';
import { IReactionFactory, REACTION_FACTORY_TOKEN, ReactionFactory } from '../../../domain/factory';
...
describe('ReactionDomainService', () => {
  ...
  describe('deleteReaction', () => {
    it('should delete post reaction', async () => {
      ...
      const result = await domainService.deleteReaction(postReactionRecord.target, postReactionRecord.id);
    ...
    it('should delete comment reaction', async () => {
      ...
      const result = await domainService.deleteReaction(commentReactionRecord.target, commentReactionRecord.id);
      expect(commentRepo.delete).toBeCalledWith(commentReactionRecord.id);
    })

    it('should throw error when target is invalid', async () => {
      jest.spyOn(postRepo, 'delete').mockRejectedValue(new Error());
      await expect(domainService.deleteReaction(postReactionRecord.target, 'invalid')).rejects.toEqual(new DatabaseException());
    });
  });
});
